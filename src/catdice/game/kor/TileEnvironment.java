...
public class TileEnvironment extends AbstractEnvironment {
	 ...
	 * @param tile
	 *            The tile to apply at the given location
	 */
	 ...
	 * 
	 * @param xp
	 *            The x position of the section to build
	 * @param yp
	 *            The y position of the section to build
	 * @param width
	 *            The width of the section to build
	 * @param height
	 *            The height of the section to build
	 */
	private void buildSection(int xp, int yp, int width, int height) {
		...
		for (int i = 0; i < combines.size(); i++) {
			...
			for (int j = 0; j < vecs.length; j++) {
				vecs[j] = new Vector2f(pts[j * 2] * tileWidth, pts[(j * 2) + 1]
						* tileHeight);
			}
	...
	/**
	 * Combine the shapes by looping combining until there are no removed
	 * shapes.
	 * 
	 * @param shapes
	 *            The shapes to combine
